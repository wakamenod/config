:PROPERTIES:
:ID:       b279fac2-fc6f-462d-8eba-94fbe582ccbb
:END:
#+title: dunst
...
* dunst (2)
http://jisyonohako.blogspot.com/2018/06/i3wmi3wmdunst.html

#+begin_src sh :tangle ~/.config/dunst/dunstrc :mkdirp yes
[global]
    ### Display ###
    ...
    #   none: 何もしない
    #
    # キーボードモードにはウィンドウマネージャが必要です
    # _NET_ACTIVE_WINDOW property.
    # これはほとんどどのウィンドウマネージャの場合も当てはまるでしょう？（わからｎ）
    #
    # mouseかkeyboardを設定していたらディスプレイの設定は無視されます
    follow = keyboard
    ...
    # offset = 1000 / 2 = 500
    #        1920 - 500 = 1420
 geometry = "1000x10-1420-100"
    # 配置的に現在隠れているメッセージの数を表示します
    indicate_hidden = yes

    # ウィンドウの幅が0よりも小再場合、縮小する。
    shrink = no

    # ウィンドウの透明度。値は [0; 100]まで。
    # このオプションはコンポジット？（透過できるやつ）が入ってないとできないよ。
    # たとえばxcompgr、compizなどなど
    transparency = 30
    ...
    icon_path = /usr/share/icons/Papirus-Dark/32x32/status
...
* dunst (1)
https://github.com/alternateved/dotfiles/blob/922f7829327c41c2ff248bedae235fd404ae8bc6/dunst/dunstrc
#+begin_src sh :tangle no :mkdirp yes
[global]
monitor = 0
follow = mouse
geometry = "380x50-24+44"
indicate_hidden = yes
shrink = no
separator_height = 4
separator_color = auto
